...
# Маппинг марок автомобилей (ID -> Название)
...
# Маппинг моделей автомобилей (model_id -> Название модели)
# Построен на основе анализа популярных моделей
MODEL_NAMES = {
    # Toyota
    1089: 'Camry',
    1128: 'Avalon',
...
if cars_data:
    ...
    for idx, car in enumerate(top_cars, 1):
        ...
        print(f"  🚗 {car['model']} | 📍 Локация {car['location']}")
...
with open("output11.txt", "w", encoding='utf-8') as f:
    ...
    for idx, car in enumerate(sorted_cars[:15], 1):
        ...
        f.write(f"Модель: {car['model']} | Локация: {car['location']}\n")
        f.write("-"*70 + "\n\n")
    ...
    for idx, car in enumerate(sorted_cars, 1):
        ...
        f.write(f"Локация: {car['location']}\n")
