...
async def evaluate_listing(session: AsyncSession, listing: CarListing) -> ListingScores:
    ...
    if ml_bundle and ml_confidence >= settings.ml_confidence_threshold:
        features = _build_ml_features(listing, market_stats, deal_result)
        expected_sell_price = predict_price(ml_bundle, features)
        pricing = compute_pricing(
            ...
            repair_cost=0.0,
            tax_rate=0.02,
            expected_sell_override=expected_sell_price,
        )
